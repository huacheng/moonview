...
| `light-exec` | ⊘ | ⊘ | ⊘ | ⊘ | ⊘ | ⊘ | =`light-exec` | →`complete` (finish) | — | →`cancelled` | PROMOTE→`planning` |
| `complete` | ⊘ | ⊘ | ⊘ | ⊘ | ⊘ | ⊘ | ⊘ | ⊘ | — (write) | ⊘ | ⊘ |
...
- `exec` requires `review` gate (cannot skip `check`) except in `light-exec` mode
- `merge` requires ACCEPT verdict gate (cannot skip `check post-exec`) or `light --finish`
- `re-planning` must pass through `check` to reach `review`
- `light-exec` can be promoted to `planning` or finished to `complete`
