...
```json
[
  { "notebook": "notebook-1", "title": "User auth refactoring" },
  { "notebook": "notebook-2", "title": "Add search functionality v2" }
]
```
...
11. **Create** `$NB_WORKSPACES_ROOT/<notebook_name>/.working/.target.md` with default template (type is auto-discovered by `research` during planning):
    ...
    ```
12. **Update** `$NB_WORKSPACES_ROOT/.index.json`: append `{ "notebook": "<notebook_name>", "title": "<title>" }` entry to the array (if not already listed)
13. **Git commit**: `ai-cli-task(<notebook_name>):init initialize notebook`
