...
| **TDD Compliance** | Medium | (software types only) Did the execution follow Red→Green→Refactor discipline? See TDD Discipline Audit below |

**TDD Discipline Audit** (software types only — `type` contains `software`):

When evaluating post-exec for software types, assess TDD compliance as an additional dimension:

1. **Red baseline exists?** — Check for `.test/<date>-red-baseline.md` (generated by `plan` step 18). If missing → deduct compliance score, note "Red baseline not generated during planning"
2. **Red→Green transitions recorded?** — Check `.notes/` exec files for TDD Cycle Summary sections. Each implementation step should have a `Red (N) → Green (N) → Refactor (yes/no)` record
3. **Cycle completeness** — Calculate: `completed_cycles / total_steps`. Acceptable threshold: ≥ 80% of steps should have completed Red→Green cycles
4. **No skipped Red checks** — Verify no steps jumped directly to Green without confirming Red first (check for "pre-Green without Red confirmation" warnings in notes)
...
- **Low** (< 50% cycles complete) → downgrade to NEEDS_FIX with guidance: "Insufficient TDD discipline — re-run failing steps with proper Red→Green verification"

Include a `## TDD Compliance` section in the `.analysis/<date>-post-exec-*.md` output file with the score and findings.
