...
3. **Send** `gemini "/moonview:auto <taskDir>"` to the session's PTY
...
2. **For each active row**:
   ...
   b. Check terminal state via `tmux capture-pane`:
      ...
      - If shell prompt visible (the agent exited) → restart with backoff: send `gemini "/moonview:auto <task_dir>"` to PTY (the agent's internal loop reads `.index.json` to determine resume point). **Restart limit**: max 3 restarts per `task_dir`. Track restart count in a new SQLite column `restart_count INTEGER DEFAULT 0`. If exceeded, set row status to `'failed'` and log error "auto loop exceeded restart limit — likely crash loop, manual intervention required". Do NOT delete the row — leave for admin inspection
