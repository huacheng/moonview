...
│   │       ├── .pending-evolutions.md   # 目标进化缓冲区（Phase 3 执行期间捕获，checkpoint 处理）
...
- `.pending-evolutions.md` is the unified objective evolution buffer — captures requirement changes (both implicit and explicit) during Phase 3 execution, processed at mid-exec/post-exec checkpoints. See `auto/SKILL.md` §Objective Evolution for the unified mechanism
