...
25. **Thinking capture** (high-value, non-blocking — failure does not block plan):
    Reflect on the design and trade-off reasoning from this planning session, then write a CoT record:
    1. Write a temp file `/tmp/thinking-plan.md` with this structure:
       ```markdown
       ---
       ...
       quality:
         thinking: <H|M|L>
         justification: "<1-sentence reason per quality-rubric.md>"
       ---
       ## CoT Capture — plan phase (<date>)
       ### Problem
       <what design question was being reasoned about>
       ### Reasoning Chain
       <key reasoning steps, alternatives considered, trade-offs>
       ### Conclusion
       <what was decided and why>
       ```
    2. Call `write_thinking_raw` helper (defined in `core/lib.sh`, path resolved via `$TASK_AI_ROOT`):
       ```bash
       write_thinking_raw plan /tmp/thinking-plan.md <notebook>
       ```
